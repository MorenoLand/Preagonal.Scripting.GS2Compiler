...
public class AntlrGrammarTests
{
	...
	{
		AssertParses("function onCreated() { if (temp.a) temp.b = 1; elseif (temp.c) temp.b = 2; else temp.b = 3; for (temp.pl: players) continue; for (; temp.i < 3; temp.i++) continue; temp.fn = function() temp.value = 1;; temp.i = int(\"3.8\"); temp.f = float(\"2.5\"); temp.t = _(\"Hello\"); temp.mask = ~temp.flags; temp.flags <<= 2; temp.flags >>= 1; temp.value ^= 3; temp.mask = temp.flags xor 3; } function emptyHook()");
	}
